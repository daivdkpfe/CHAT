...
conn.listen({
    ...
    onFileMessage: function ( message ) {
        console.log(message);
            ...
            var img="<div class='message-file-top'>文件</div><div class='message-file-bottom'><img class='file_img' src='images/message_file.png' alt=''><p class='file_name'>"+message.filename+"</p><p class='file_down_url'><a href='"+message.url+"'>下載</a></p></div>";
            if(message['delay']){
                var time=UTC(message['delay']);
            }
            else
        ...
        success: function ( roster ) {
            ...
            for ( var i = 0, l = roster.length; i < l; i++ ) {
                ...
                else
                {
                    ...
                    if ( ros.subscription === 'none') {
                        friendList= friendList +'<div class="contect contect_'+roster[i]["name"]+'"> <div class="contect_logo"> <img src="images/portrait.jpg" alt=""> </div> <p class="contect_name">'+roster[i]["name"]+'</p><d>（陌生人）</d></div>';
            ...
            if(getData.to!=null)
            {
                ...
                else
                {
                    ...
                    friendList= friendList +'<div class="contect contect_'+getData.to+'"> <div class="contect_logo"> <img src="images/portrait.jpg" alt=""> </div> <p class="contect_name">'+getData.to+'</p><d>（陌生人）</d></div>';
                    $(".contect_list").html(friendList);
                }
   ...
   // console.log(emojibody);
    ...
    for(var i = 0 , l = data.length ; i < l ; i++){
      ...
      //  var username=$(".select-id").find(".chat-username").html(); //找到發送給哪個用戶
        ...
        if(type==2)
        {
            $(".chat_list").find("."+sendUser).find(".contect_zuijin").html("[ 圖片 ]");
        }
        else if(type==3)
        {
            $(".chat_list").find("."+sendUser).find(".contect_zuijin").html("[ 文件 ]");
        }
        else
        {
    ...
    $.ajax({ url: "../chat?action=query",
        ...
        success: function(req){
            ...
            for(var i=0; i<req.length; i++){
                ...
                {
                    $(".history_record_div").append("<p class='no_history-record-list'>無歷史記錄</p>")
                }
        ...
        var option = {
            ...
            success: function () {                // 消息发送成功
                $.ajax({ url: "../chat",
                    ...
                    success: function(req){
                        var img="<div class='message-file-top'>File</div><div class='message-file-bottom'><img class='file_img' src='images/message_file.png' alt=''><p class='file_name'>"+msg.filename+"</p><p class='file_down_url'><a href='"+msg.body.body.url+"'>下載</a></p></div>";
