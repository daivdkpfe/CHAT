...
function addReceiveMessage(message,from,time) {
    ...
    if($(".chat-list-div").length)//好友列表已加載
    {
        ...
        {
            $(".chat-div-main-top").html($(".chat-div-main-top").html()+'<div class="chat-list-div  chat-'+from+'"><img src="images/portrait.jpg" class="chat-portrait" alt=""><p class="chat-username">'+from+'</p><p class="isNoFriend">（陌生人）</p><p class="recent-message"></p></div>');
        ...
        if(type==2)
        {
            $(".chat-"+from).find(".recent-message").html("[ 圖片 ]");
        }
        else if(type==3)
        {
            $(".chat-"+from).find(".recent-message").html("[ 文件 ]");
        }
        else if(type==4)
        {
            $(".chat-"+from).find(".recent-message").html("[ 語音 ]");
        }
        else
        {
    ...
    $.ajax({ url: "../chat?action=query",
        ...
        success: function(req){
            ...
            for(var i=0; i<req.length; i++){
                ...
                {
                    $(".history-record-list").append("<p class='no_history-record-list'>無历史记录</p>")
                }
        ...
        var option = {
            ...
            success: function () {                // 消息发送成功
                $.ajax({ url: "../chat",
                    ...
                    success: function(req){
                        var img="<div class='message-file-top'>文件</div><div class='message-file-bottom'><img class='file_img' src='images/message_file.png' alt=''><p class='file_name'>"+msg.filename+"</p><p class='file_down_url'><a href='"+msg.body.body.url+"'>下載</a></p></div>";
